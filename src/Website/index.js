...
const { WEBSITE, DEVELOPMENT_MODE } = require('../config').Config;
...
const WebsiteOnline = new WebhookClient({ id: WEBSITE.WEBHOOKS.ONLINE.ID, token: WEBSITE.WEBHOOKS.ONLINE.TOKEN });
...
app.get('/servers', require('./Utils/Routes/servers'));
app.get('/dashboard', require('./Utils/Routes/dashboard'));

app.get('/dashboard/information', require('./Utils/Routes/information'));
app.get('/dashboard/customize', require('./Utils/Routes/customize'));
app.get('/dashboard/settings', require('./Utils/Routes/settings'));
...
app.listen(WEBSITE.PORT, () => { 
  ...
  if (!DEVELOPMENT_MODE) {
    WebsiteOnline.send({
      embeds: [{ color: '', author: { name: 'statify' }, description: 'Website is now online using proxy port 3000', timestamp: new Date().toLocaleString() }]
